...
async function fetchBannedWords() {
    try {
        ...
        const bannedWordsRange = 'Заборонені!A:B'; // name_uk, name_ru

        const response = await fetch(`${API_BASE}/api/sheets/public?range=${encodeURIComponent(bannedWordsRange)}`);
        ...
        const csvData = values.map(row => row.join(',')).join('\n');
