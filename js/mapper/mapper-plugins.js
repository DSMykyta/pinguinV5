// js/mapper/mapper-plugins.js

/**
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║                    MAPPER - PLUGIN SYSTEM                                ║
 * ╚══════════════════════════════════════════════════════════════════════════╝
 *
 * Система хуків для модульної архітектури Mapper.
 * Дозволяє плагінам реєструвати callbacks на певні події.
 *
 * 🔒 ЯДРО — цей файл не можна видаляти!
 */

/**
 * Реєстр хуків
 */
const hooks = {
    onInit: [],              // Після завантаження даних
    onCategoryAdd: [],       // Після додавання категорії
    onCategoryUpdate: [],    // Після оновлення категорії
    onCategoryDelete: [],    // Після видалення категорії
    onCharAdd: [],           // Після додавання характеристики
    onCharUpdate: [],        // Після оновлення характеристики
    onCharDelete: [],        // Після видалення характеристики
    onOptionAdd: [],         // Після додавання опції
    onOptionUpdate: [],      // Після оновлення опції
    onOptionDelete: [],      // Після видалення опції
    onMarketplaceAdd: [],    // Після додавання маркетплейсу
    onMarketplaceUpdate: [], // Після оновлення маркетплейсу
    onMarketplaceDelete: [], // Після видалення маркетплейсу
    onImport: [],            // Після імпорту
    onMapping: [],           // Після маппінгу
    onRender: [],            // Після рендеру таблиці
    onTabChange: [],         // Після зміни активного табу
};

/**
 * Зареєструвати плагін на хук
 * @param {string} hookName - Назва хука
 * @param {Function} callback - Функція для виклику
 * @param {number} priority - Пріоритет (менше = раніше), default 10
 */
export function registerMapperPlugin(hookName, callback, priority = 10) {
    if (!hooks[hookName]) {
        console.warn(`[Mapper Plugins] Невідомий хук: ${hookName}`);
        return;
    }

    hooks[hookName].push({ callback, priority });
    hooks[hookName].sort((a, b) => a.priority - b.priority);
}

/**
 * Виконати всі callbacks для хука
 * @param {string} hookName - Назва хука
 * @param {...any} args - Аргументи для передачі в callbacks
 */
export function runHook(hookName, ...args) {
    if (!hooks[hookName]) {
        console.warn(`[Mapper Plugins] Невідомий хук: ${hookName}`);
        return;
    }

    hooks[hookName].forEach(({ callback }) => {
        try {
            callback(...args);
        } catch (err) {
            console.error(`[Mapper Plugin Error] ${hookName}:`, err);
        }
    });
}

/**
 * Асинхронно виконати всі callbacks для хука
 * @param {string} hookName - Назва хука
 * @param {...any} args - Аргументи для передачі в callbacks
 */
export async function runHookAsync(hookName, ...args) {
    if (!hooks[hookName]) {
        console.warn(`[Mapper Plugins] Невідомий хук: ${hookName}`);
        return;
    }

    for (const { callback } of hooks[hookName]) {
        try {
            await callback(...args);
        } catch (err) {
            console.error(`[Mapper Plugin Error] ${hookName}:`, err);
        }
    }
}

/**
 * Опціональні функції, які можуть бути зареєстровані плагінами
 */
export const optionalFunctions = {};

/**
 * Зареєструвати опціональну функцію
 * @param {string} name - Назва функції
 * @param {Function} func - Функція
 */
export function registerOptionalFunction(name, func) {
    optionalFunctions[name] = func;
}
