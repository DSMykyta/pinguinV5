...
import {
    getOptions, getMpOptions,
    createOptionMapping, isMpOptionMapped
} from './mapper-data.js';
    ...
    mpItems.forEach(mpItem => {
        listHtml += `
            <label style="display:flex;align-items:center;gap:10px;padding:8px 12px;border:1px solid var(--color-border);border-radius:var(--radius-m);cursor:pointer;">
                ...
                <span class="chip chip-active">${escapeHtml(mpItem.marketplaceName)}</span>
            </label>`;
    });
    listHtml += '</div>';
