// js/pages/entities/entities-mapping-wizard-options.js

/**
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║              ENTITIES - OPTION MAPPING WIZARD                            ║
 * ╠══════════════════════════════════════════════════════════════════════════╣
 * ║  Покроковий візард для маппінгу MP опцій до власних.                   ║
 * ║                                                                        ║
...
import { createOptionMapping, isMpOptionMapped } from '../../data/mappings-data.js';
...
function getMpOptionName(mpOption) {
    let name = extractName(mpOption);
    if (name) return name;
    if (mpOption.data) {
        try {
            const d = typeof mpOption.data === 'string' ? JSON.parse(mpOption.data) : mpOption.data;
            name = extractName(d);
        } catch (e) { /* ignore */ }
    }
    return name || mpOption.external_id || mpOption.id;
}

function getMarketplaceName(mpId) {
    const mp = getMarketplaces().find(m => m.id === mpId);
...
function buildAllCards() {
    ...
    const unmapped = mpOptions.filter(o => !isMpOptionMapped(o.id));
    ...
    unmapped.forEach(mpOpt => {
        const name = getMpOptionName(mpOpt);
        ...
        groups.get(key).mpItems.push({
            ...
            marketplaceName: getMarketplaceName(mpOpt.marketplace_id),
            name: name
        });
    });

    // Шукаємо відповідні власні опції
    const ownByName = new Map();
    ownOptions.forEach(opt => {
        const key = normalizeName(opt.value_ua);
        if (key && !ownByName.has(key)) ownByName.set(key, opt);
    });

    const cards = [];
    groups.forEach(group => {
        const ownOpt = ownByName.get(group.normalizedName) || null;
        if (!ownOpt) return;

        group.ownItem = ownOpt;
        cards.push(group);
    });
...
function renderFilterPhase() {
    ...
    if (title) title.textContent = 'Маппінг опцій';
    ...
    body.innerHTML = `
        <div style="margin-bottom:16px;">
            ...
            <input type="text" class="input-main" id="wizard-search" placeholder="Наприклад: червоний, 100 мл..." value="${escapeHtml(wizardState.searchQuery)}" />
        </div>
        <div style="margin-bottom:16px;">
...
function renderCard() {
    ...
    const charInfo = card.ownItem.characteristic_id
        ? `<div style="font-size:12px;color:var(--color-text-secondary);margin-top:2px;">Характеристика: ${escapeHtml(card.ownItem.characteristic_id)}</div>`
        : '';
    ...
    const headerHtml = `
        <div style="padding:12px 16px;background:var(--color-surface-variant);border-radius:var(--radius-m);margin-bottom:16px;">
            ...
            ${charInfo}
            <div style="font-size:16px;font-weight:600;margin-top:2px;">${escapeHtml(card.ownItem.value_ua)}</div>
    ...
    mpItems.forEach(mpItem => {
        listHtml += `
            <label style="display:flex;align-items:center;gap:10px;padding:8px 12px;border:1px solid var(--color-border);border-radius:var(--radius-m);cursor:pointer;">
                ...
                <span class="chip c-main">${escapeHtml(mpItem.marketplaceName)}</span>
            </label>`;
    });
    listHtml += '</div>';
...
async function applyCurrentCard() {
    ...
    if (!selectedMpIds.length) {
        showToast('Оберіть хоча б одну опцію', 'warning');
        return;
    }
    ...
    try {
        const ownOptionId = card.ownItem.id;

        for (const mpId of selectedMpIds) {
            await createOptionMapping(ownOptionId, mpId);
...
export async function showOptionMappingWizard() {
    const allCards = buildAllCards();

    if (!allCards.length) {
        showToast('Немає MP опцій з відповідними власними для маппінгу', 'info');
        return;
    }
