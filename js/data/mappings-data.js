 ...
 * ╚══════════════════════════════════════════════════════════════════════════╝
 *
 * Спільний data layer для маппінгів (прив'язка власних сутностей до MP сутностей).
 * CRUD операції, перевірки (isMapped), batch операції, автоматичний маппінг.
...
import { getMpCategories, getMpCharacteristics, getMpOptions } from './mp-data.js';
...
/**
 * Завантажити маппінги категорій
 */
export async function loadMapCategories() {
    try {
        ...
        _mapCategories = rows.map((row, index) => {
        ...
        _mapCategories = await deduplicateMappings(
            'MAP_CATEGORIES',
            _mapCategories,
...
export async function loadMapCharacteristics() {
    try {
        ...
        _mapCharacteristics = rows.map((row, index) => {
            const obj = { _rowIndex: index + 2 };
            headers.forEach((header, i) => {
                obj[header] = row[i] || '';
            });
        ...
        _mapCharacteristics = await deduplicateMappings(
            'MAP_CHARACTERISTICS',
            _mapCharacteristics,
            m => `${m.characteristic_id}|${m.mp_characteristic_id}`
        );

        return _mapCharacteristics;
    } catch (error) {
...
export async function loadMapOptions() {
    try {
        ...
        _mapOptions = rows.map((row, index) => {
            const obj = { _rowIndex: index + 2 };
            headers.forEach((header, i) => {
                obj[header] = row[i] || '';
            });
        ...
        _mapOptions = await deduplicateMappings(
            'MAP_OPTIONS',
            _mapOptions,
...
/**
 * Отримати всі маппінги характеристик
 */
...
/**
 * Отримати всі маппінги опцій
 */
export function getMapOptions() {
    return _mapOptions;
}

// Setters (для polling — зовнішнє оновлення стану)
export function setMapCategories(data) { _mapCategories = data; }
export function setMapCharacteristics(data) { _mapCharacteristics = data; }
export function setMapOptions(data) { _mapOptions = data; }

// ═══════════════════════════════════════════════════════════════════════════
// КАТЕГОРІЇ — CHECK & GET MAPPED
// ═══════════════════════════════════════════════════════════════════════════
...
export function isMpCategoryMapped(mpCatId) {
    ...
    const inMappingTable = _mapCategories.some(m =>
...
export function getMappedMpCategories(ownCatId) {
    ...
    const mappings = _mapCategories.filter(m =>
...
export function isMpCharacteristicMapped(mpCharId) {
    ...
    const inNewTable = _mapCharacteristics.some(m =>
...
export function getMappedMpCharacteristics(ownCharId) {
    ...
    const mappings = _mapCharacteristics.filter(m =>
...
export function getCharacteristicMappingByMpId(mpCharId) {
    return _mapCharacteristics.find(m => m.mp_characteristic_id === mpCharId);
}
...
export function isMpOptionMapped(mpOptionId) {
    ...
    const inNewTable = _mapOptions.some(m =>
...
export function getMappedMpOptions(ownOptionId) {
    ...
    const mappings = _mapOptions.filter(m =>
...
export function getOptionMappingByMpId(mpOptionId) {
    return _mapOptions.find(m => m.mp_option_id === mpOptionId);
}
...
export async function createCategoryMapping(ownCatId, mpCatId) {
    try {
        const existing = _mapCategories.find(m =>
            m.category_id === ownCatId && m.mp_category_id === mpCatId
        );
        if (existing) return existing;

        const newId = generateId('map-cat', _mapCategories);
        ...
        const newMapping = {
            ...
            _rowIndex: getNextRowIndex(_mapCategories)
        };
        _mapCategories.push(newMapping);
        return newMapping;
    } catch (error) {
...
export async function deleteCategoryMapping(mappingId) {
    try {
        const mapping = _mapCategories.find(m => m.id === mappingId);
        if (!mapping) {
        ...
        const index = _mapCategories.findIndex(m => m.id === mappingId);
        if (index !== -1) {
            _mapCategories.splice(index, 1);
        }
        adjustRowIndices(_mapCategories, rowIndex);
    } catch (error) {
...
export async function deleteCategoryMappingByMpId(mpCatId) {
    const mapping = _mapCategories.find(m => m.mp_category_id === mpCatId);
    if (mapping) {
...
export async function createCharacteristicMapping(ownCharId, mpCharId) {
    try {
        const existing = _mapCharacteristics.find(m =>
            m.characteristic_id === ownCharId && m.mp_characteristic_id === mpCharId
        );
        if (existing) return existing;

        const newId = generateId('map-char', _mapCharacteristics);
        const timestamp = new Date().toISOString();
        ...
        const newMapping = {
            ...
            created_at: timestamp,
            _rowIndex: getNextRowIndex(_mapCharacteristics)
        };
        _mapCharacteristics.push(newMapping);
        return newMapping;
    } catch (error) {
...
export async function deleteCharacteristicMapping(mappingId) {
    try {
        const mapping = _mapCharacteristics.find(m => m.id === mappingId);
        if (!mapping) {
            throw new Error(`Маппінг ${mappingId} не знайдено`);
        }
        ...
        const index = _mapCharacteristics.findIndex(m => m.id === mappingId);
        if (index !== -1) {
            _mapCharacteristics.splice(index, 1);
        }
        adjustRowIndices(_mapCharacteristics, rowIndex);
    } catch (error) {
...
export async function deleteCharacteristicMappingByMpId(mpCharId) {
    const mapping = _mapCharacteristics.find(m => m.mp_characteristic_id === mpCharId);
    if (mapping) {
...
export async function createOptionMapping(ownOptionId, mpOptionId) {
    try {
        const existing = _mapOptions.find(m =>
            m.option_id === ownOptionId && m.mp_option_id === mpOptionId
        );
        if (existing) return existing;

        const newId = generateId('map-opt', _mapOptions);
        const timestamp = new Date().toISOString();
        ...
        const newMapping = {
            ...
            created_at: timestamp,
            _rowIndex: getNextRowIndex(_mapOptions)
        };
        _mapOptions.push(newMapping);
        return newMapping;
    } catch (error) {
...
export async function deleteOptionMapping(mappingId) {
    try {
        const mapping = _mapOptions.find(m => m.id === mappingId);
        if (!mapping) {
            throw new Error(`Маппінг ${mappingId} не знайдено`);
        }
        ...
        const index = _mapOptions.findIndex(m => m.id === mappingId);
        if (index !== -1) {
            _mapOptions.splice(index, 1);
        }
        adjustRowIndices(_mapOptions, rowIndex);
    } catch (error) {
...
export async function deleteOptionMappingByMpId(mpOptionId) {
    const mapping = _mapOptions.find(m => m.mp_option_id === mpOptionId);
    if (mapping) {
